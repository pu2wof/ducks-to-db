...
let insertObservation = (obs) => {
  return new Promise((resolve, reject) => {
    if (!obs || !obs.timestamp ||
      !obs.deviceType || !obs.deviceID || 
      !obs.latitude || !obs.longitude) {
      return reject({"err":error_msgs.missing_observation_fields, "status": 400})
    }

    let current_date = new Date().toISOString();
    let sent_timestamp = parseInt(obs.timestamp);
    let timestamp = new Date(sent_timestamp).toISOString();
    let device_type = obs.deviceType;
    let device_id = obs.deviceID;
    let latitude = obs.latitude;
    let longitude = obs.longitude;
