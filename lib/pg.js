...
let insertObservation = (obs) => {
  return new Promise((resolve, reject) => {
    if (!obs.observation || !obs.observation.timestamp ||
      !obs.observation.device_type || !obs.observation.device_id || 
      !obs.observation.latitude || !obs.observation.longitude) {
      return reject({"err":error_msgs.missing_observation_fields, "status": 400})
    }

    let observ = obs.observation;
    let current_date = new Date().toISOString();
    let timestamp = observ.timestamp;
    let device_type = observ.device_type;
    let device_id = observ.device_id;
    let latitude = observ.latitude;
    let longitude = observ.longitude;
