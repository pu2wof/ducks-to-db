...
let saveTestReport = (info, results) => {
  return new Promise((resolve, reject) => {
    let current_date = new Date().toISOString();
    let sql = `INSERT INTO ${test_reports_table}
      (name, description, results, created_at, updated_at)
      VALUES ('${info.name}', '${info.description}', '${JSON.stringify(results)}',
      '${current_date}','${current_date}');`
    db.none(sql).then(_ => {
      resolve({"info": info, "results": results})
    }).catch(err => {
      console.log(err)
      reject(err)
    })
...
let checkMessages = (req, res, next, io) => {
  ...
  db.any(sql).then(db_results => {
    ...
    // save test report
    saveTestReport(info, results).then(report => {
