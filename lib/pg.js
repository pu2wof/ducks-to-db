...
let saveTestReport = (info, results, hit_score) => {
  return new Promise((resolve, reject) => {
    let current_date = new Date().toISOString();
    let sql = `INSERT INTO ${test_reports_table}
      ...
      '${hit_score}', '${current_date}','${current_date}');`
    console.log(sql)
    db.none(sql).then(_ => {
      resolve({"name": info, "results": results, "message_success_rate": hit_score})
    }).catch(err => {
      console.log(err)
      reject(err)
    })
...
let checkMessages = (req, res, next, io) => {
  ...
  db.any(sql).then(db_results => {
    ...
    let hit_score = 0+"%"
    if ((hit + miss) > 0) {
      hit_score = (hit / (hit + miss)*100)+"%"
    }

    // save test report
    saveTestReport(info, results, hit_score).then(report => {
